...
int DFT(int idft, double *xr, double *xi, double *Xr_o, double *Xi_o, int N) {
    ...
    typedef struct { double sum_r; double sum_i; char pad[128]; } padded_sum;

    for (int k = 0; k < N; k++) {

        #pragma omp parallel
        {
            padded_sum partial_sum;
            partial_sum.sum_r = 0.0;
            partial_sum.sum_i = 0.0;

            #pragma omp for
            for (int n = 0; n < N; n++) {

                // Real part of X[k]
                partial_sum.sum_r += xr[n] * cos(n * k * PI2 / N) + idft * xi[n] * sin(n * k * PI2 / N);
                // Xr_o[k] += xr[n] * cos(n * k * PI2 / N) + idft * xi[n] * sin(n * k * PI2 / N);

                // Imaginary part of X[k]
                partial_sum.sum_i += -idft * xr[n] * sin(n * k * PI2 / N) + xi[n] * cos(n * k * PI2 / N);
                /// Xi_o[k] += -idft * xr[n] * sin(n * k * PI2 / N) + xi[n] * cos(n * k * PI2 / N);
            }

            #pragma omp critical
            {
                Xr_o[k] += partial_sum.sum_r;
                Xi_o[k] += partial_sum.sum_i;
            }
